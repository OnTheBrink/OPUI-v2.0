...
function Module:SendZoneList()
	...
	if output ~= "" and _G[db.zone.frame] then
		...
		_G[db.zone.frame]:AddMessage(("|cff00ff96%s%s:|r %s"):format(zone, percent or "", output:sub(1, -2)))
	end
end
...
function Module:PetChatCommand(args)
	...
	if action == "unique" then
		...
		-- then print it
		self:Print((L["You have |cff00ff96%d|r unique pets."]):format(count))
	...
	elseif action == "duplicates" then
		...
		self:Print((L["You have |cff00ff96%d|r duplicate pets."]):format(count))
	...
	elseif action == "unowned" then
		...
		self:Print((L["You are missing |cff00ff96%d|r pets."]):format(count))
		...
		self:Print((L["You have |cff00ff96%d (%d unique, %d duplicates)|r pets."]):format(totalPets, unique, dupes))
	end
end

function Module:MountChatCommand(args)
	self:Print((L["You have |cff00ff96%d|r mounts."]):format(GetNumCompanions("MOUNT")))
end
