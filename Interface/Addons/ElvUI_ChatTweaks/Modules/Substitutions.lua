...
function Module:GetKeywords()
	local str = ""
	local keyword = "%s |cff00ff96(%s)|r, "
	for index, value in pairs(self.patterns) do
		str = str .. keyword:format(value.name, index:gsub("%%%%", "%%"))
	end
...
function Module:GetColumn(index)
	...
	local subString = "%s |cff00ff96(%s)|r"
	...
	for i = start, finish do
		out = out .. space .. subString:format(patterns[i].name, patterns[i].pat)
		if i ~= finish then
			out = out .. "\n"
		end
	end
