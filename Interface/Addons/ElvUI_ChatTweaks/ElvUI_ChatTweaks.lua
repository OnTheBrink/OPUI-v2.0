...
ElvUI_ChatTweaks.addon = (L["|cff1784d1ElvUI Chat Tweaks|r version |cff1784d1%s|r"]):format(ElvUI_ChatTweaks.version)
...
function ElvUI_ChatTweaks:EnableAllModules()
	for name, module in self:IterateModules() do
		if self.db.profile.debugging then
			self:Print((L["Enabled |cff00ff96%s|r module."]):format(name))
		end
		module:Enable()
	end
end
...
function ElvUI_ChatTweaks:DisableAllModules()
	for name, module in self:IterateModules() do
		if self.db.profile.debugging then
			self:Print((L["Disabled |cff00ff96%s|r module."]):format(name))
		end
		module:Disable()
	end
end

function ElvUI_ChatTweaks:PLAYER_ENTERING_WORLD(self, event, ...)
	if ElvUI_ChatTweaks.db.profile.welcome == true then
		print(ElvUI_ChatTweaks.addon .. L[" is loaded. Type |cff1784d1/ct|r if you need help."])
	end
	...
	ElvUI_ChatTweaks.NotifyTimer = ElvUI_ChatTweaks:ScheduleTimer("SendMessage", 12)
	ElvUI_ChatTweaks:UnregisterEvent("PLAYER_ENTERING_WORLD")
end
...
function ElvUI_ChatTweaks:SendMessage()
	...
	if ElvUI_ChatTweaks.db.profile.debugging then
		ElvUI_ChatTweaks:Print((L["Addon Version Sent, Version: |cffffff00%s|r"]):format(tostring(ElvUI_ChatTweaks.version)))
	end
	
	if inInstance and instanceType == "pvp" or instanceType == "arena" then
		SendAddonMessage("ElvUI_ChatTweaks", ElvUI_ChatTweaks.version, "INSTANCE_CHAT")
	else
		if IsInRaid() and GetNumGroupMembers() > 0 then
			SendAddonMessage("ElvUI_ChatTweaks", ElvUI_ChatTweaks.version, IsPartyLFG() and "INSTANCE_CHAT" or "RAID")
		elseif not IsInRaid() and GetNumSubgroupMembers() > 0 then
			SendAddonMessage("ElvUI_ChatTweaks", ElvUI_ChatTweaks.version, IsPartyLFG() and "INSTANCE_CHAT" or "PARTY")
		elseif IsInGuild() then
			SendAddonMessage("ElvUI_ChatTweaks", ElvUI_ChatTweaks.version, "GUILD")
		end
	end
	
	if ElvUI_ChatTweaks.NotifyTimer then
		ElvUI_ChatTweaks:CancelTimer(ElvUI_ChatTweaks.NotifyTimer)
		ElvUI_ChatTweaks.NotifyTimer = nil
	end
end

function ElvUI_ChatTweaks:CHAT_MSG_ADDON(self, event, prefix, message, channel, sender)
	if not ElvUI_ChatTweaks.db.profile.notify then return end
	if sender == UnitName("player") then return end
	
	if prefix == "ElvUI_ChatTweaks" and not ElvUI_ChatTweaks.receivedOutOfDateMessage then
		if tonumber(message) ~= nil and tonumber(message) > tonumber(ElvUI_ChatTweaks.version) then
			if ElvUI_ChatTweaks.db.profile.debugging then ElvUI_ChatTweaks:Print((L["Your Version: |cffffff00%d|r, Latest Version: |cffffff00%d|r"]):format(tonumber(ElvUI_ChatTweaks.version), tonumber(message))) end
			ElvUI_ChatTweaks:Print((L["Your version of %s is out of date.  Latest version is |cff1784d1%d|r."]):format(ElvUI_ChatTweaks.addon, message))
			ElvUI_ChatTweaks.receivedOutOfDateMessage = true	-- to prevent receiving this message more than once
		end
	end
end
