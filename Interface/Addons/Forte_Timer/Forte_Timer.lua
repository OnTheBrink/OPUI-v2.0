...
local function NewStatusBar(parent)
  ...
  if not bar.bg then
		...
		bar.bg:Point("TOPRIGHT", bar, "TOPRIGHT", 2, 2)
		bar.bg:Point("BOTTOMLEFT", bar, "BOTTOMLEFT", -2, -2)
...
local function NewBar(parent,n)
	...
	bar.Update = function(self)
      ...
      if not self.button.bg then
				self.button.bg = CreateFrame("Frame", nil, bar)
				self.button.bg:SetTemplate("Transparent")
				self.button.bg:Point("TOPRIGHT", self.button, "TOPRIGHT", 2, 2)
				self.button.bg:Point("BOTTOMLEFT", self.button, "BOTTOMLEFT", -2, -2)
				self.button.bg:SetFrameLevel(self.button:GetParent():GetFrameLevel() - 1)
			end
			if s.IconRight then -- icon on right side of bar
        self.button:SetPoint("TOPLEFT",self,"TOPRIGHT",5,0);
				self.button:SetPoint("BOTTOMLEFT",self,"BOTTOMRIGHT",5,0);
			else -- icon on left side of bar
				self.button:SetPoint("TOPRIGHT",self,"TOPLEFT",-5,0);
				self.button:SetPoint("BOTTOMRIGHT",self,"BOTTOMLEFT",-5,0);
			end
			self.button:Show();
		else
			self.button:Hide();
