...
local function NewBar(parent,n)
  ...
  --bar:SetFrameLevel(1)	-- layer order	
	-- icon
	bar.icon = CreateFrame("Button",nil,bar);
	bar.icon:SetPoint("CENTER",bar,"CENTER",0,0);
  bar.icon:SetFrameLevel(10 + 2*n)
  ...
  if not bar.icon.bg then
		bar.icon.bg = CreateFrame("Frame", nil, bar)
		bar.icon.bg:SetTemplate("Default")
		bar.icon.bg:Point("TOPRIGHT", bar.icon, "TOPRIGHT", 2, 2)
		bar.icon.bg:Point("BOTTOMLEFT", bar.icon, "BOTTOMLEFT", -2, -2)
		bar.icon.bg:SetFrameLevel(bar.icon:GetFrameLevel())
	--print(n, bar.icon:GetFrameLevel(), bar.icon.bg:GetFrameLevel())
	--bar.icon.bg:SetBackdropBorderColor( 0.07, 0.65, 1)
	--bar.icon.bg:SetBackdropBorderColor(r, g, b)
...
local function CD_NewCooldownFrame(index)
  ...
  if not frame.back.bg then
		frame.back.bg = CreateFrame("Frame", nil, frame)
		frame.back.bg:Point("TOPRIGHT", frame.back, "TOPRIGHT", 2, 2)
		frame.back.bg:Point("BOTTOMLEFT", frame.back, "BOTTOMLEFT", -2, -2)
		frame.back.bg:SetTemplate("Transparent")
		--frame.back.bg:CreateShadow("Default")
		frame.back.bg:SetFrameLevel(2)
	end
